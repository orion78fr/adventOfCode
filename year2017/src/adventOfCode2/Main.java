...
public class Main {
    public static void main(String[] args) {
        String in = "104\t240\t147\t246\t123\t175\t372\t71\t116\t230\t260\t118\t202\t270\t277\t292\n" +
        ...
        for (String line : in.split("\n")) {
            ...
            for (int i = 0; i < split.length-1; i++) {
                int intI = Integer.parseInt(split[i]);
                for (int j = i+1; j < split.length; j++) {
                    int intJ = Integer.parseInt(split[j]);
                    int rem = intI < intJ ? intJ % intI : intI % intJ;
                    if (rem == 0) {
                        sum2 += intI < intJ ? intJ / intI : intI / intJ;
                    }
                }
            }
        }
