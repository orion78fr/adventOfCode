...
public class Main {
    public static void main(String[] args) {
        String in = "vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh\n" +
        ...
        long count = Arrays.asList(in.split("\n")).stream().map(s -> Arrays.asList(s.split(" ")))
                .map(l -> l.stream().collect(Collectors.toMap(Function.identity(), x -> 1, (u, v) ->u + v)))
                .map(m -> m.entrySet().stream().anyMatch(e -> e.getValue() > 1))
                .filter(b -> !b)
                .count();

        System.out.println(count);

        long count2 = Arrays.asList(in.split("\n")).stream().map(s -> Arrays.asList(s.split(" ")))
                .map(l -> l.stream().collect(Collectors.toMap(Main::sortString, x -> 1, (u, v) ->u + v)))
                .map(m -> m.entrySet().stream().anyMatch(e -> e.getValue() > 1))
                .filter(b -> !b)
                .count();
