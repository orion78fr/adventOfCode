...
public class Main {
    ...
    public static void main(String[] args) {
        Map<Integer, List<Integer>> links = Arrays.stream(in.split("\n"))
                .map(str -> Arrays.stream(str.split("[ <\\->,]+")).mapToInt(Integer::parseInt).boxed().collect(Collectors.toList()))
        ...
        Queue<Integer> pidsToVisit = new LinkedList<>();
        pidsToVisit.add(0);

        while(!pidsToVisit.isEmpty()){
            Integer pid = pidsToVisit.poll();
            if(!visitedPids.contains(pid)){
                visitedPids.add(pid);
                pidsToVisit.addAll(links.get(pid));
            }
        }

        System.out.println(visitedPids.size());

        int group = 1;
        while(visitedPids.size() != links.size()){
            group++;

            Optional<Integer> first = links.keySet().stream().filter(pid -> !visitedPids.contains(pid)).findFirst();

            pidsToVisit.add(first.get());
